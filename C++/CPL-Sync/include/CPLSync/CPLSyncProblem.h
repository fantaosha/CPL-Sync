 ...
 * of the semidefinite relaxation solved by CPL-Sync.
 *
...
class CPLSyncProblem {
public:
  /// PROBLEM DATA

  /** The specific retraction of the CPL-Sync problem used) */
  Retraction rect_;

  /** The specific formulation of the CPL-Sync problem to be solved
(translation-implicit, translation-explicit, or robust) */
  ...
  /** The matrices B1, B2, and B3 defined in equation (69) of the CPL-Sync tech
   * report */
  ComplexSparseMatrix B1_, B2_, B3_;

  /** The matrix M parameterizing the quadratic form appearing in the Explicit
   * form of the CPL-Sync problem (Problem 2 in the CPL-Sync tech report) */
  ComplexSparseMatrix M_;

  /** The rotational connection Laplacian for the special Euclidean
   * synchronization problem, cf. eq. 14 of the CPL-Sync tech report.  Only
   * used in Implicit mode.*/
  ComplexSparseMatrix LGrho_;

  /** The weighted reduced oriented incidence matrix Ared Omega^(1/2) (cf.
   * eq. 39 of the CPL-Sync tech report).  Only used in Implicit mode. */
   ...
   * of the CPL-Sync tech report.  Only used in Implicit mode. */
...
public:
  ...
  /** Returns the specific formulation of this CPL-Sync problem */
  ...
  /** Returns the StiefelProduct manifold underlying this CPL-Sync problem */
  ...
  objective  F(Y) := tr(S * Y' * Y) for the CPL-Sync problem.  More precisely:
  *
  * If formulation == Implicit, this returns Q * Y, where Q is as defined in
  equation (24) of the CPL-Sync tech report.
  *
  * If formulation == Explicit, this returns M * Y, where M is as defined in
  equation (18) of the CPL-Sync tech report. */
  ...
  /** Given a matrix Y in the domain D of the CPL-Sync optimization problem, the
  ...
  /** Given a point Y in the domain D of the rank-r relaxation of the CPL-Sync
  ...
  /** Given a critical point Y of the rank-r relaxation of the CPL-Sync
  ...
  struct SMinusLambdaProdFunctor {
    ...
    // Constructor
    SMinusLambdaProdFunctor(const CPLSyncProblem *prob, const ComplexMatrix &Y,
                            Scalar sigma = 0);
