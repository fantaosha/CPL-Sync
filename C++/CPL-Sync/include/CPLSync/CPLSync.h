/** This file provides a convenient functional interface to the CPLSync
 * algorithm.
 *
...
/** This struct contains the various parameters that control the CPLSync
 * algorithm */
struct CPLSyncOpts {
  ...
  std::experimental::optional<CPLSyncTNTUserFunction> user_function;

  /// CPL-SYNC PARAMETERS

  Retraction retraction = Retraction::Projection;

  /** The specific formulation of the CPL-Sync problem to solve */
   ...
   * initialization strategy employed by CPL-Sync: 'true' -> chordal, 'false' ->
   * random sampling */
  ...
  /** If this value is true, the CPL-Sync algorithm will log and return the
...
/** These enumerations describe the termination status of the CPL-Sync algorithm
 */
enum CPLSyncStatus {
...
/** This struct contains the output of the CPLSync algorithm */
struct CPLSyncResult {

  /** An estimate of a global minimizer Yopt of the rank-restricted dual
   * semidefinite relaxation Problem 9 in the CPL-Sync tech report.  The
   ...
   * according to eq. (119) in the CPL-Sync tech report.  If Z = Y^T Y is an
  ...
  /** The total elapsed computation time for the CPL-Sync algorithm */
  ...
  /** If CPL-Sync was run with log_iterates = true, this will contain the
   * sequence of iterates generated by the truncated-Newton trust-region method
   * at each level of the Riemannian Staircase */
  std::vector<std::vector<ComplexMatrix>> iterates;

  /** The termination status of the CPL-Sync algorithm */
  CPLSyncStatus status;
};

/** Given an CPLSyncProblem instance, this function performs synchronization */
CPLSyncResult CPLSync(CPLSyncProblem &problem,
                      const CPLSyncOpts &options = CPLSyncOpts(),
                      const ComplexMatrix &Y0 = ComplexMatrix());

/** Given a vector of relative pose measurements specifying a special Euclidean
 * synchronization problem, performs synchronization using the CPLSync algorithm
 */
